...
class Node:
  ...
  def evaluate(self, a_list: List[List[int]], p_list: List[List[int]]) -> Tuple[bool, int, int, int]:  # 訪問済みか、開始時刻、終了時刻、ジョブID
    if not self.operation:
      return (float('inf'), float('inf'), float('inf'), float('inf'))

    last_job_id, last_operation_index = self.operation  # 今回のノードで追加されたオペレーション
    # machine_id = a_list[last_job_id][last_operation_index]  # 使用するマシンID  今回は使用しない
    processing_time = p_list[last_job_id][last_operation_index]  # 処理時間
    end_time = self.job_available_time[last_job_id]  # オペレーションの終了時刻
    start_time = end_time - processing_time  # オペレーションの開始時刻
    return (self.is_visited, start_time, end_time, last_job_id)
